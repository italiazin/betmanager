...
def _post_discord(msg_id, texto, img_bytes=None):
    """Chamado logo após cada envio pro Discord — salva no Supabase."""
    if int(msg_id) not in _sup_salvos:
        threading.Thread(
            target=_sup_insert,
            args=(msg_id, texto),
