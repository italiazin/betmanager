...
print("VERSAO_CARREGADA: OCR_CLIENTE_V123_LIMPA_JOGO_STATUS_PRECISO")
...
def ultimas_apostas_comunidade_base(limit=10):
    ...
    for b in list(reversed(dados.get("bets", []))):
        ...
        # Feed da comunidade: só apostas públicas originais/OCR/manual.
        # Cópias nunca entram no feed, mesmo se alguma flag vier errada.
        if not b.get("publica", False) or not b.get("user_id"):
            continue
        if b.get("origem") == "copia" or b.get("source_original_id"):
            continue
...
def criar_aposta_manual_payload(form):
    ...
    bet = {
        ...
        "estado": "",
        "lucro": 0,
        "origem": "copia" if form.get("copiada_comunidade") == "1" else "manual",
        "source_original_id": form.get("source_original_id", ""),
        "mercado": classificacao.get("mercado", "Outro"),
        "direcao": classificacao.get("direcao", ""),
        "linha": classificacao.get("linha", None),
        "periodo": classificacao.get("periodo", "jogo inteiro"),
...
def normalizar_float(v):
    try:
        ...
        if isinstance(v, (int, float)):
            return float(v)
        s = str(v).strip().replace("R$", "").replace("RS", "").replace(" ", "")
        if "," in s and "." in s:
            s = s.replace(".", "").replace(",", ".")
        else:
            s = s.replace(",", ".")
    ...
    try:
        ...
        for a in apostas:
            try:
                ...
                # Se o OCR não trouxe valor, mantém 0 para o usuário editar depois.
                if odd <= 0:
                    odd = 0
                ...
                bet = {
                    ...
                    "lucro": 0,
                    "origem": "ocr",
                    "mercado": a.get("mercado") or classificacao.get("mercado", "Outro"),
                    "direcao": classificacao.get("direcao", ""),
                    "linha": a.get("linha") if a.get("linha") not in ["", None] else classificacao.get("linha", None),
                    "periodo": a.get("periodo") or classificacao.get("periodo", "jogo inteiro"),
                    "selecao": a.get("selecao") or classificacao.get("selecao", ""),
                    "btts_resposta": classificacao.get("btts_resposta", ""),
                    "api_status": "",
                    "texto_bruto": a.get("texto_bruto", ""),
                    "texto_interpretado": a.get("texto_interpretado", aposta_texto),
        ...
        print("ERRO GERAL salvar_preview:", repr(e))
        return jsonify({"ok": False, "erro": str(e)}), 500
