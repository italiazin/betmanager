...
def _backup_diario_loop():
    """Thread que roda para sempre e gera backup JSON local todo dia às 03:00."""
    import time
    ...
    while True:
        ...
        try:
            ...
            if conn:
                ...
                if row:
                    path = os.path.join(BACKUP_DIR, f"dados_{ts}.json")
                    with open(path, "w", encoding="utf-8") as f:
                        f.write(row[0])
                if row_u:
                    path_u = os.path.join(BACKUP_DIR, f"usuarios_{ts}.json")
                    with open(path_u, "w", encoding="utf-8") as f:
                        f.write(row_u[0])

                # Mantém apenas os últimos 7 dias
                arquivos = sorted(
                    [fn for fn in os.listdir(BACKUP_DIR) if fn.endswith(".json")],
                    reverse=True
                )
                # Agrupa por data (dados_ e usuarios_ separados)
                datas_dados = sorted(
                    set(fn.replace("dados_", "").replace(".json", "") for fn in arquivos if fn.startswith("dados_")),
                    reverse=True
                )
                for data_antiga in datas_dados[7:]:
                    for prefixo in ("dados_", "usuarios_"):
                        alvo = os.path.join(BACKUP_DIR, f"{prefixo}{data_antiga}.json")
                        if os.path.exists(alvo):
                            os.remove(alvo)
                            print(f"Backup antigo removido: {alvo}")

                print(f"Backup diário JSON criado: dados_{ts}.json")
        except Exception as e:
